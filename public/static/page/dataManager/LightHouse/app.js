var
  ...
  db_host='http://ch.mryitao.cn/?user=default',
  current_database,
  current_table,
  editor,
  resp_field_types,
...
function query(key, str, callback) {
  ...
  params += "&password=" + encodeURIComponent('5hc1Zd24');
  ...
  if (basic_user) {
  xhr.open("POST", uri.replace(/basic_user\=([^&]+)/g, ''), false, basic_user, host_password);
  ...
  xhr.onreadystatechange = function() {
    ...
    if (xhr.readyState === XMLHttpRequest.DONE) {
        if (xhr.status === 200) {
          ...
          if(current_database){
            window.allData[1]=res;

          }else{
            window.allData[0]=res;
          }
          callback({
            fields: fields,
            rows: res.data,
...
function selectDatabase(val, first) {
  ...
  //  console.log(ClickhouseHighlightRules)
  // init();
  // editor = ace.edit("query");
  if(!first){
    // init();
    ...
    // window.init1();
    // window.init3("ace/mode/clickhouse","./clickhouse_highlight_rules").ClickhouseHighlightRules
    // editor.session.setMode("ace/mode/clickhouse");
    // let oop = require("./js/ace-min/clickhouse_highlight_rules");
    // console.log(oop)
    window.inherits(window.ClickhouseHighlightRules,window.TextHighlightRules);
    var ClickhouseHighlightRules = window.ClickhouseHighlightRules;
    var lx=new window.i()
    ClickhouseHighlightRules.apply(lx);
  }
