...
(function ($) {
    ...
    $.fn.Flowdesign = function (options) {
        ...
        if ($.browser && $.browser.msie && $.browser.version < '9.0') { //ie9以下，用VML画图
        ...
        jsPlumb.bind("jsPlumbConnection", function (info) {
            var sourceId = info.source.attr("process_id");
            var targetId = info.target.attr("process_id");
            if(typeof jsPlumb._enforceList[sourceId+"_"+targetId] == "undefined" || jsPlumb._enforceList[sourceId+"_"+targetId] == "true"){
                info.connection.setLabel("强依赖");
            }else{
                info.connection.setLabel("弱依赖");
            }
        ...
        jsPlumb.bind("click", function (c) {
            $.showModal({
                content:"请进行删除连接或者切换连接类型(强依赖/弱依赖)操作",
