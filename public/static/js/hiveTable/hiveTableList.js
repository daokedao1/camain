require(['jquery','jquery.bootstrap','jquery.datetimepicker','common','quickSearch','app'],function($){
    ...
    var hiveTableList = {
        ...
        deleteTable:function(id,dbName,name){
            ...
            $.ajax({
                type: "post",
                url: "/venus/hiveManage/dropTable",
                dataType:"json",
                    ...
                    if(result && result.success){
                        ...
                        for(var i=0;i<results.length;i++){
                            var sHtml = '<div class="box box-solid collapsed-box" data-pid="'+results[i].id+'">'
                                ...
                                +'</div>';
                            $("#leftKu").append(sHtml);
                            if(_this.pid == null && i == 0){
                                _this.pid = results[i].id;
                            }
                        }
                    ...
                    if(result && result.success){
                        ...
                        for(var r in pairs){
                            ...
                            for(var i=0;i<secondElms.length;i++){
                                ...
                                sHtml = sHtml + '<li><a href="javascript:void(0);" data-id="'+secondElms[i].id+'"><i class="fa fa-star '+((_this.hdb == secondElms[i].id)?"text-light-blue":"")+'"></i>'+secondElms[i].name+'</a></li>'
                            }
                            sHtml = sHtml + '</ul>'
                                +'</div>'
                                +'</div>';
